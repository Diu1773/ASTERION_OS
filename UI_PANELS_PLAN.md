...
- [x] **U6** 풀리뷰(16에이전트 4차원+적대검증) + 회귀. ✅출시가능(must-fix 0). 고위험 9건 오탐 기각.
  should-fix 5건 반영: 기상소스 NaN가드·잘못된utc 시각미상·STALE 전용색(st-stale)·예보 aria-label·로드실패 빈렌더.
  회귀: node-check·등록정합성(3패널×4)·TABS 7개·create_app 110라우트·버전 일치(149/180).
