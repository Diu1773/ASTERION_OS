...
> **ASTREION OS**는 장비·기상·데이터·안전·스케줄·분석·AI 판단을 하나의 상태 체계로 통합하는 자율형 천문 관측 운영체제이다.  
> 이 문서는 ASTREION OS의 전체 논리 구조, 세션 분리, 핵심 모듈, 개발 우선순위를 정리한 로드맵이다.
...
ASTREION OS는 단순한 웹 대시보드가 아니다.  
장비 연결 프로그램도 아니고, 자동촬영 스크립트도 아니다.

ASTREION OS의 본질은 다음과 같다.
...
ASTREION Console
ASTREION Command
ASTREION Device Settings
ASTREION Target Page
ASTREION Skyview

        ↓

[Operation Layer]
ASTREION Orchestrator
ASTREION Meridian
ASTREION Watchtower
ASTREION Policy Engine

        ↓

[State / Knowledge Layer]
ASTREION State Store
ASTREION Event Bus
ASTREION Skygraph
ASTREION Provenance
...
[Device / Data Layer]
ASTREION Device Manager
ASTREION Device Core
ASTREION Telemetry
ASTREION Weather Store
ASTREION Archive
ASTREION Calibration Library
ASTREION Forge
ASTREION Sentinel
...
ASTREION Agent Runtime
ASTREION PC Agent
ASTREION Ingestion API
ASTREION Plugin Runtime
ASTREION Access
ASTREION Alert
ASTREION Simulator / Twin
...
ASTREION OS 개발은 하나의 거대한 기능으로 진행하면 안 된다.  
...
Device Session은 천문 장비를 ASTREION OS에 연결하고, 각 장비를 관측 시스템 안의 역할로 배정하는 계층이다.

중요한 원칙은 다음과 같다.

> **UI가 장비에 직접 맞춰지는 것이 아니라, 장비가 ASTREION의 표준 모델에 맞춰 번역되어야 한다.**

예를 들어 RST 가대, ZWO 필터휠, Moravian 카메라, PlaneWave PWI, 기상센서, 포커서, 돔 컨트롤러는 연결 방식이 서로 다르지만 ASTREION 내부에서는 표준 상태 모델로 표현되어야 한다.
...
장비는 단순히 연결되는 것이 아니라 ASTREION 안에서 역할을 가져야 한다.
...
ASTREION은 저수준 기능을 관측 행위로 변환해야 한다.
...
State Session은 ASTREION OS의 현재 상태를 관리하는 계층이다.
...
Event Session은 ASTREION 내부 모듈들이 서로 느슨하게 연결되도록 하는 신경망이다.
...
Orchestrator는 ASTREION OS의 관측 실행 지휘자이다.
...
Orchestrator가 없으면 ASTREION은 장비 버튼 모음에 머문다.  
...
ASTREION에서는 데이터가 왜, 어떻게, 어떤 조건에서 만들어졌는지까지 기록되어야 한다.
...
Skygraph는 ASTREION OS의 천문 관측 온톨로지이다.
...
AI Agent는 ASTREION OS에 나중에 붙는 해석·추천 계층이다.
...
UI / UX Session은 사용자가 ASTREION OS를 실제로 조작하고 이해하는 화면을 구성한다.
...
| 화면 | 역할 | 디자인 참고 |
|---|---|---|
| ASTREION Home | 프로젝트 소개 | Raycast + Linear |
| ASTREION Console | Muuri 기반 통합 관측 패널 | NASA Open MCT + Linear |
| ASTREION Command | 명령 팔레트 | Raycast |
...
Infrastructure Session은 ASTREION OS가 실제 관측소 환경에서 동작하기 위한 서버, PC Agent, API, 권한, 플러그인 구조를 담당한다.
...
장기적으로 ASTREION OS는 다양한 장비와 분석 모듈을 받아들여야 한다.
...
목표는 코드 작성 전에 ASTREION의 뼈대를 고정하는 것이다.
...
이 단계부터 ASTREION이 단순 웹앱이 아니라 OS처럼 보이기 시작한다.
...
ASTREION을 연구 플랫폼으로 확장할 수 있는 데이터 기반을 만든다.
...
ASTREION을 단순 대시보드에서 관측 실행 시스템으로 발전시킨다.
...
ASTREION OS의 핵심은 장비를 많이 연결하는 것이 아니다.  
...
이 다섯 개가 잡히면 ASTREION OS는 단순한 통합 대시보드가 아니라 자율 관측 운영체제로 발전할 수 있다.
